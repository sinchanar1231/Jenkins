pipeline {
    ...
    stages{
        ...
        stage('Create image'){
            steps{
                sh 'sudo docker build -t app /var/lib/jenkins/workspace/AP_project/' //jobname
            }
        }
        stage('Assign tag'){
            steps{
                sh 'docker tag app sinchana1231/app'
            }
        }
        stage('Push to dockerhub'){
            steps{
                sh 'echo "Mydocker@12" | docker login -u "sinchana1231" --password-stdin'
                sh 'docker push sinchana1231/app'
            }
        }
        stage('Remove images'){
            steps{
        ...
        stage('Pull image from DockerHub'){
            steps{
                sh 'docker pull sinchana1231/app'
            }
        }
        stage('Run a container'){
            steps{
                sh 'docker run -it -d --name sinchu -p 8081:8080 sinchana1231/app'
            }
        }
    }
    post {
